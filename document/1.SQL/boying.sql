...
CREATE TABLE `menu`  (
  ...
  `sort` int(11) NOT NULL DEFAULT 1 COMMENT '菜单排序',
...
CREATE TABLE `review`  (
  ...
  `show_id` int(11) NOT NULL COMMENT '该评论所属演出Id',
  `star` int(11) NOT NULL COMMENT '对演出打出的星级',
...
CREATE TABLE `role`  (
  ...
  `sort` int(11) NOT NULL DEFAULT 1 COMMENT '用于排序',
...
CREATE TABLE `user`  (
  ...
  `default_frequent` int(11) NULL COMMENT '默认观影者',
  `default_address` int(11) NULL COMMENT '默认地址',
...
CREATE TABLE `user_order`  (
  ...
  `show_session_id` int(11) NOT NULL COMMENT '所属场次Id',
  `address_id` int(11) NULL COMMENT '这些票要邮寄到什么地方',
  `frequent_id` int(11) NULL COMMENT '这些票的实际观演人',
  `status` int(11) NOT NULL COMMENT '待评价,已完成(1,2)',
